...
ToView is capable of estimating planes, cylinders and spheres. The process also allows the user to further refine the primitives selecting more points after the initial estimation.

This estimation is also performed in real-time and is easily accessible from the GUI. When the user clicks on the option to segment primitive (see \autoref{tools_menu}), a new dialog will appear that will let the user fine tune the segmentation options (see \autoref{segm_opt}). The process is really intuitive for the user, as it is only needed to select a point of the object that needs to be segmented.

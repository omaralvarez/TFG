...
As programmable shaders arrived, we now have the possibility to perform the MSAA resolve in a custom shader. In this project this step was not deemed a priority and we have relied on the OpenGL API to perform this step. Summing up, the use of MSAA has given us an increase in image quality without a big impact in performance. This increase in rendering quality is specially noticeable in dynamic cases, but can also be observed in static images (see \autoref{msaa_comp}).  

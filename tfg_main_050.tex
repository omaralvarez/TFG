...
In the diagram of \autoref{conv_man}, the following classes are the most relevant:
\begin{itemize}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{figures/conv_man.pdf}
...
PCM uses a spatial data structure (a kd-tree) as core of the
multi-resolution system. The relevant classes are shown in the class
diagram of \autoref{tree_class}. The most important class is
\textbf{PCTree}. This class encapsulates two of the main
functionalities of the multi-resolution system:
\begin{itemize}
...
\begin{figure}
	\centering
	\includegraphics[width=0.81\textwidth]{figures/tree_class.pdf}
...
As an implementation detail, it is relevant to remark that the tree nodes, represented by the class \textbf{PCTreeNode}, are all kept in a lineal vector for quick access ($O(1)$). There are also some functions valid for any binary tree related to the traversal of the tree; getting the child nodes of a parent, level of a node, etc. There are also functions to read/write the tree from/to disk. Finally, there are also functions to build priority node lists for the visualizer.


\subsection[Memory hierarchy]{Memory hierarchy}

The most significant classes in charge of the management of the memory
hierarchy are shown in the shown in the class diagram of \autoref{hier_class}:
\begin{itemize}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{figures/hier_class.pdf}
...
\begin{figure}
	\centering
	\includegraphics[scale=0.65]{figures/external_interf.pdf}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{figures/chunk.pdf}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.6]{figures/mem_hierch.pdf}
...
\lstset{language=C++,frame=shadowbox,rulesepcolor=\color[gray]{0.8},lineskip=10pt, 
		...
		emph={PointCloud}, emphstyle=\color{Emerald}\bfseries, basicstyle=\footnotesize,
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{figures/L2.pdf}
...
\\
\lstset{emph={PointCloud,Chunk}, emphstyle=\color{Emerald}\bfseries, basicstyle=\footnotesize,
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{figures/node_sharing.pdf}
...
\section[Results]{\label{sec:res_pcm}Results}
...
The operating system used to run the tests was Windows 7 x64, with the VC{}\verb!++! v110 compiler. The libraries used were:
...
\midrule
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/castelao.png}} & Castelao & $\sim 500K$  & Color, Normals \\[1cm]
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/dorna.png}} & Dorna & $\sim 3M$  & Color \\[1cm]
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/oficina.png}} & Oficina & $\sim 30M$  & Color \\[1cm]
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/lourizan.png}} & Lourizan & $\sim 90M$  & Color \\[1cm]
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/CALD.png}} & CALD & $\sim 200M$  & Color \\[1cm]
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/baron.png}} & Baron & $\sim 500M$  & Color \\[1cm]
 \parbox{2.5cm}{\includegraphics[width=\linewidth]{figures/castelo.png}} & Castelo & $\sim 1000M$  & Color \\
\bottomrule
\end{tabular}
...
\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/cpu_gpu_est_cast_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/cpu_gpu_est_cast_780.pdf}}
...
\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/cpu_gpu_est_dorna_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/cpu_gpu_est_dorna_780.pdf}}
...
In addition to this compute tests, cache performance will be tested on the most demanding datasets available. The first dataset that will tested is Lourizan. In \autoref{l1_hits_chunk_vbo}, we can observe how L1 cache hits evolve depending on chunk size or VBO size. 

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_780.pdf}}
	\caption[Hits versus chunk size versus VBO size in Lourizan]{
		Graphs showing how hit rate on L1 behaves depending on chunk size and VBO size in Lourizan.
	}
	\label{l1_hits_chunk_vbo}
\end{figure}

We can see that the cache behaves better when using big VBO sizes, but this is not the whole picture. To have a real idea of the optimal cache parameters, we need to take a look at \autoref{l1_time_chunk_vbo}. This test compares load times against chunk sizes and VBO sizes. 

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_780.pdf}}
	\caption[Load time vs. chunk size vs. VBO size in Lourizan]{
		Graphs showing how load times on L1 evolve depending on chunk size and VBO size in Lourizan.
	}
	\label{l1_time_chunk_vbo}
\end{figure}

What can be deduced from this test, is that loading bigger chunk sizes in GPU memory takes more time in general, this was an expected result. What is most interesting about this second test, is the fact that we can see there is a sweet spot between 1-4 MB when loading VBOs. The loading time increases as we augment the VBO size, when trying to load the same amount of information. This is why we cannot just use the biggest VBO size available to increase hit rate. 

Summing up, the optimal parameters are using an intermediate chunk size, and a VBO size between 1-4 MB. Besides testing the L1 cache, we have also tested the l2 cache. In this case, the first test performed compares the hit rate depending on the chunk size chosen (see \autoref{chunk_hits_lour}).   

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_lour_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_lour_4930k.pdf}}
	\caption[Chunk size vs. hit rate in Lourizan]{
		Graphs showing how hit rate on L2 evolves depending on chunk size in Lourizan.
	}
	\label{chunk_hits_lour}
\end{figure}

On the aforementioned test, that for some chunk sizes the faster machine had worse results came as a surprise. As we will see in \autoref{chunk_misses_lour}, since the cache is never full using this configuration, this may be one of the reasons the weaker machine was faster on some cases. But taking all tests into account, L1 and L2, the faster computer still comes out on top.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_lour_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_lour_4930k.pdf}}
	\caption[Chunk size vs. miss rate in Lourizan]{
		Graphs showing how the miss rate on L2 evolves depending on chunk size in Lourizan.
	}
	\label{chunk_misses_lour}
\end{figure}

The last test performed, compares L2 loading time depending on chunk size (see \autoref{chunk_time_lour}). As we can see in the graph, the optimal chunk size depends on the machine. But it can be said with some confidence, that a size of $\sim 100K$ points is a safe bet when choosing the chunk size.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_lour_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_lour_4930k.pdf}}
	\caption[Chunk size vs. load time in Lourizan]{
		Graphs showing how the load times on L2 vary depending on chunk size in Lourizan.
	}
	\label{chunk_time_lour}
\end{figure}

The next dataset tested was CALD, in which the same tests were performed. Therefore, the first test performed was \autoref{l1_hits_chunk_vbo_cald}, in which we can see how L1 cache hits evolve depending on chunk size and VBO size. 

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_cald_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_cald_780.pdf}}
	\caption[Hits versus chunk size versus VBO size in CALD]{
		Graphs showing how hit rate on L1 behaves depending on chunk size and VBO size in CALD.
	}
	\label{l1_hits_chunk_vbo_cald}
\end{figure} 

Next, the test that compares load times against chunk sizes and VBO sizes (see \autoref{l1_time_chunk_vbo_cald}). The tests performed in L1 corroborate the results of the tests performed with Lourizan. The only difference that can be seen, is the fact that in Lourizan the faster computer was able to load every chunk requested with a big array of configurations (see \autoref{l1_hits_chunk_vbo} and \autoref{l1_hits_chunk_vbo_cald}). With this dataset the graph resembles more the graph of the slower machine.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_cald_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_cald_780.pdf}}
	\caption[Load time vs. chunk size vs. VBO size in CALD]{
		Graphs showing how load times on L1 evolve depending on chunk size and VBO size in CALD.
	}
	\label{l1_time_chunk_vbo_cald}
\end{figure}

The next test performed compares the hit and miss rate depending on the chunk size chosen in L2 (see \autoref{chunk_hits_CALD} and \autoref{chunk_misses_CALD}).   

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_cald_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_cald_4930k.pdf}}
	\caption[Chunk size vs. hit rate in CALD]{
		Graphs showing how hit rate on L2 evolves depending on chunk size in CALD.
	}
	\label{chunk_hits_CALD}
\end{figure}

These results mimic the last results obtained when using the Lourizan dataset. Although, these are not the best datasets to test the L2 cache, since they are not big enough to fill the cache. The two remaining datasets will be true stress tests for the L2 cache when using this configuration.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_cald_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_cald_4930k.pdf}}
	\caption[Chunk size vs. miss rate in CALD]{
		Graphs showing how the miss rate on L2 evolves depending on chunk size in CALD.
	}
	\label{chunk_misses_CALD}
\end{figure} 

The last test compares L2 loading time depending on chunk size (see \autoref{chunk_time_cald}). As we can see in the graph, the stronger machine is faster for every chunk size. The only chunk size in which the weaker machine can compete is the biggest one tested.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_cald_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_cald_4930k.pdf}}
	\caption[Chunk size vs. load time in CALD]{
		Graphs showing how the load times on L2 vary depending on chunk size in CALD.
	}
	\label{chunk_time_cald}
\end{figure}

The next dataset tested was Baron, in which the same battery of tests were performed. As a consequence, the first test performed was \autoref{l1_hits_chunk_vbo_baron}, in which we can see how L1 cache hits evolve depending on chunk size and VBO size. 

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_baron_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_baron_780.pdf}}
	\caption[Hits versus chunk size versus VBO size in Baron]{
		Graphs showing how hit rate on L1 behaves depending on chunk size and VBO size in Baron.
	}
	\label{l1_hits_chunk_vbo_baron}
\end{figure} 

Next, the test that compares load times against chunk sizes and VBO sizes (see \autoref{l1_time_chunk_vbo_baron}). The tests performed in L1 corroborate the results of the tests performed with the Lourizan and CALD datasets. No further conclusions have been reached with this new dataset.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_baron_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_baron_780.pdf}}
	\caption[Load time vs. chunk size vs. VBO size in Baron]{
		Graphs showing how load times on L1 evolve depending on chunk size and VBO size in Baron.
	}
	\label{l1_time_chunk_vbo_baron}
\end{figure}

The next test performed compares the hit and miss rate of the L2 cache depending on the chunk size chosen (see \autoref{chunk_hits_baron} and \autoref{chunk_misses_baron}).

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_baron_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_baron_4930k.pdf}}
	\caption[Chunk size vs. hit rate in Baron]{
		Graphs showing how hit rate on L2 evolves depending on chunk size in Baron.
	}
	\label{chunk_hits_baron}
\end{figure}

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_baron_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_baron_4930k.pdf}}
	\caption[Chunk size vs. miss rate in Baron]{
		Graphs showing how the miss rate on L2 evolves depending on chunk size in Baron.
	}
	\label{chunk_misses_baron}
\end{figure} 

These results mimic the results obtained when using the Lourizan and CALD datasets. The stronger machine clearly has a big advantage, that when dealing with bigger datasets increases even further. But this is the first dataset where 4 GB of memory is not enough to store the complete dataset. This is the reason why we start to see a lot of capacity misses, that eventually surpass the compulsory misses. This is the first big test for our L2 cache, that even when full keeps an acceptable hit rate.

The last test compares L2 loading time depending on chunk size (see \autoref{chunk_time_baron}). As we can see in the graph, the stronger machine is faster for every chunk size. The weaker machine is not able to compete anymore when dealing with the Baron dataset.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_baron_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_baron_4930k.pdf}}
	\caption[Chunk size vs. load time in Baron]{
		Graphs showing how the load times on L2 vary depending on chunk size in Baron.
	}
	\label{chunk_time_baron}
\end{figure}

To sum up, the gap between Computer 1 and Computer 2 gets bigger as the dataset sizes increase. This behavior was expected since the tree gets bigger and cache performance becomes more crucial. The bigger 12 MB cache in the CPU also should help the faster machine achieve better results. 

The last dataset tested was Castelo, in which once more, the same battery of tests were performed. Consequently, the first test performed was \autoref{l1_hits_chunk_vbo_castelo}, in which we can see how L1 cache hits change depending on chunk size and VBO size. 

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_castelo_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_hits_chunk_vbo_castelo_780.pdf}}
	\caption[Hits versus chunk size versus VBO size in Castelo]{
		Graphs showing how hit rate on L1 behaves depending on chunk size and VBO size in Castelo.
	}
	\label{l1_hits_chunk_vbo_castelo}
\end{figure} 

Next, the test that compares load times against chunk sizes and VBO sizes (see \autoref{l1_time_chunk_vbo_castelo}). The tests performed in L1 corroborate the results of the tests performed with the Lourizan, CALD and Baron datasets. No further conclusions have been reached by testing this new dataset.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_castelo_640.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/l1_time_chunk_vbo_castelo_780.pdf}}
	\caption[Load time vs. chunk size vs. VBO size in Castelo]{
		Graphs showing how load times on L1 evolve depending on chunk size and VBO size in Castelo.
	}
	\label{l1_time_chunk_vbo_castelo}
\end{figure}

The next test performed compares the hit and miss rate of the L2 cache depending on the chunk size chosen (see \autoref{chunk_hits_castelo} and \autoref{chunk_misses_castelo}).

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_castelo_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_hits_castelo_4930k.pdf}}
	\caption[Chunk size vs. hit rate in Castelo]{
		Graphs showing how hit rate on L2 evolves depending on chunk size in Castelo.
	}
	\label{chunk_hits_castelo}
\end{figure}

These results gain confirm the results obtained when using the Lourizan, CALD and Baron datasets. The stronger machine clearly has a big advantage, as this is the most demanding dataset tested in this round of testing. The advantage is so big, that even in the first sample capacity misses are greater that compulsory misses. As a consequence, we do not see the graphed miss lines cross at any point. 

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_castelo_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_misses_castelo_4930k.pdf}}
	\caption[Chunk size vs. miss rate in Castelo]{
		Graphs showing how the miss rate on L2 evolves depending on chunk size in Castelo.
	}
	\label{chunk_misses_castelo}
\end{figure} 

The last test compares L2 loading time depending on chunk size (see \autoref{chunk_time_castelo}). As we can see in the graph, Computer 2 is faster for every chunk size beating Computer 1 by a big margin. The weaker machine was not able to compete in the Baron dataset and with the Castelao dataset is even worse.

\begin{figure}[h]
	\centering
	\subfigure[Computer 1]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_castelo_3770.pdf}}
	\subfigure[Computer 2]{\includegraphics[width=0.49\textwidth]{figures/chunk_time_castelo_4930k.pdf}}
	\caption[Chunk size vs. load time in Castelo]{
		Graphs showing how the load times on L2 vary depending on chunk size in Castelo.
	}
	\label{chunk_time_castelo}
\end{figure}

In conclusion, PCM scales well when dealing with massive point clouds. Of course, when running on machines with weaker hardware, there will be a dip in performance. But, our objective was satisfied, since PCM was able to work with massive datasets on low and mid-range machines. This fact will be better seen on \autoref{sec:tov_results}, since a Linux laptop was used for the tests performed. 

All of the operations that PCM is able to perform are out of core, this means that all of the operations run in the tests are applicable to clouds of arbitrary sizes. This is specially valuable when dealing with the extremely dense clouds that scanners provide nowadays. The biggest cloud that has been tested in PCM has been an artificially generated $\sim 12KM$ point cloud. This cloud is orders of magnitude bigger than the clouds that point cloud software is able to deal with nowadays. This makes PCM an unique piece of software in its class, that is prepared even for future datasets that are even more demanding.   

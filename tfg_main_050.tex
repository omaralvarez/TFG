...
At first sight, on the aforementioned test, one might think that the faster machine had worse results. As we will see in \autoref{chunk_misses_lour}, since the cache is never full, it means quite the opposite. The faster computer was able to load a lot more chunks, therefore, the compulsory misses are greater and the hit rate is lower.
...
The next dataset tested was CALD, with the same tests as Lourizan. Therefore, the first test performed was \autoref{l1_hits_chunk_vbo_cald}, in which we can see how L1 cache hits evolve depending on chunk size and VBO size. 
